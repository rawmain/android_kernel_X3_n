...
* GLobal Variable
*****************************************************************************/
static struct i2c_board_info lp3101_board_info __initdata = { I2C_BOARD_INFO(I2C_ID_NAME, TPS_ADDR) };
...
int lp3101_write_bytes(unsigned char addr, unsigned char value)
{
	...
	if (client == NULL) {
		pr_err("ERROR!!lp3101_i2c_client is null\n");
		return 0;
	}
...
static void lcm_init(void)
{
	...
	if (ret < 0)
		pr_err("[KERNEL]r63419----lp3101---cmd=%0x-- i2c write error-----\n", cmd);
	else
		pr_debug("[KERNEL]r63419----lp3101---cmd=%0x-- i2c write success-----\n", cmd);
#endif
